...
RUN apt-get update && \
    apt-get upgrade -y && \
    apt-get install --no-install-recommends -y ca-certificates tzdata gzip bzip2 xz-utils unzip \
    build-essential cmake curl wget git vim ffmpeg libopenblas-dev libssl-dev && \
...
RUN --mount=type=cache,target=/root/.cache/uv \
    uv venv --python 3.12 && \
    uv pip install --torch-backend=auto torch torchaudio humanfriendly funasr
...
RUN cd websocket && mkdir build && cd build && \
    ...
    -DONNXRUNTIME_DIR=/workspace/FunASR/onnxruntime-linux-x64-1.14.0 \
